 ...
 void diep(char *s)
  ...
  int main(void)
  {
   ...
   freeaddrinfo(result);           /* No longer needed */
     ...
     char *tmp1=(char *)malloc(sizeof(char)*30);
     ...
     //bzero(tmp1,sizeof(tmp1));
     i=getline(&tmp1,&tam,stdin);
     tmp1[strlen(tmp1)-1]='\0';
     strncpy(o1->apellido,tmp1, strlen(tmp1));
     //gets(o1->apellido);
     ...
     if (sendto(cd, (const char*)o1, sizeof(struct dato), 0, (struct sockaddr *)rp->ai_addr, rp->ai_addrlen)<=0)
        diep("sendto()");  
     free(o1);
    close(cd);
    return 0;

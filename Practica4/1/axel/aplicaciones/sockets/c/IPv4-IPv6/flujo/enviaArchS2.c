...
*/	
	    char *arch1="/home/axel/aplicaciones/aplicaciones/flujo/duke1.png";
	    //char *arch1="/home/escom/Escritorio/hola.txt";
            char buffer[100];
	    ...
	    char* ts1 = strdup(arch1);
	      ...
	      sprintf(metainfo,"1_%s;%ld",nombre,tamanio);
              //enviados=write(cd,metainfo,strlen(metainfo)+1);
	      enviados=write(cd,metainfo,sizeof(metainfo));
	      //fflush(f1);
              if(enviados<0){
 		 ...
 		 printf("1: %d\n",l);
		   ...
		   return 1;
	         ...
	         //fflush(f1);
	         printf("\rEnviados: %ld de %ld bytes.. %ld ",escritos,tamanio,leidos);
                 if((l == EOF )|| (l == 0)) {
                    printf("\nfin del archivo\n");
                    break;
                 }//if
